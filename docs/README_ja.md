...
このリポジトリはサーバーと Web UI の 1.2.5-fix2、および Agent 1.2.60 を固定して統合しています。インストール、更新、リリース手順は [リポジトリの README](../README.md) を参照してください。
...
コンテナ環境では、このリポジトリの [Docker Compose 設定](../compose.yaml) とサーバーイメージ `ghcr.io/kadidalax/komari-classic:latest` を使用してください。
...
Go 1.25+、Node.js 22+、CGO 対応の C コンパイラが必要です。このリポジトリをクローンし、ルートからローカルの `frontend/` をビルドしてサーバーに埋め込んだ後、サーバーと `agent/` をビルドします。コマンドとバージョン指定は [README のビルド手順](../README.md#构建源码) を参照してください。
