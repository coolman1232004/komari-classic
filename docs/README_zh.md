...
本仓库固定合并服务端和网页 1.2.5-fix2、探针 1.2.60。完整安装、更新和发布说明见 [仓库首页](../README.md)。
...
容器平台部署请使用本仓库的 [Docker Compose 配置](../compose.yaml)，服务端镜像为 `ghcr.io/kadidalax/komari-classic:latest`。
...
需要 Go 1.25+、Node.js 22+ 和支持 CGO 的 C 编译器。先克隆本仓库，在根目录构建本地 `frontend/` 并嵌入服务端，然后编译服务端和 `agent/`。完整命令及版本参数见 [仓库首页的构建步骤](../README.md#构建源码)。
