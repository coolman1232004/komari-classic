...
本倉庫固定合併服務端與網頁 1.2.5-fix2、探針 1.2.60。完整安裝、更新與發行說明見 [倉庫首頁](../README.md)。
...
容器平台部署請使用本倉庫的 [Docker Compose 設定](../compose.yaml)，服務端映像為 `ghcr.io/kadidalax/komari-classic:latest`。
...
需要 Go 1.25+、Node.js 22+ 與支援 CGO 的 C 編譯器。先複製本倉庫，在根目錄建置本機 `frontend/` 並嵌入服務端，再編譯服務端與 `agent/`。完整指令與版本參數見 [倉庫首頁的建置步驟](../README.md#构建源码)。
